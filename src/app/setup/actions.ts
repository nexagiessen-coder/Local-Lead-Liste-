...
export async function setupAction(_prev: SetupState, formData: FormData): Promise<SetupState> {
  ...
  if ((row?.n ?? 0) > 0) return { error: 'Setup has already been completed. Sign in instead.' };
  ...
  try {
    ...
    await recordAudit({ userId: user.id, action: 'setup.completed', entityType: 'user', entityId: user.id });
  } catch (error) {
