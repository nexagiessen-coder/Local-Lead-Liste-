...
describe('research pipeline', () => {
  it('discovers, verifies and pools businesses without creating leads', async () => {
    ...
    const leads = await listBusinessRows({ scope: 'leads' }, db);
    expect(leads).toHaveLength(0);

    const pool = await listBusinessRows({ scope: 'pool' }, db);
  ...
  it('stores an evidence trail for every verified business', async () => {
    ...
    for (const row of pool) {
      ...
      const verification = await getLatestVerification(row.business.id, db);
...
describe('qualification gate', () => {
  it('refuses to promote a business that is not verified as having no website', async () => {
    ...
    await expect(promoteToLead(withWebsite!.business.id, userId, {}, db)).rejects.toThrow(QualificationBlockedError);
  });
  ...
  it('allows an explicit override and marks it as overridden', async () => {
    ...
    const lead = await promoteToLead(blocked!.business.id, userId, { force: true }, db);
