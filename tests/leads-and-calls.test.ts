...
describe('lead list', () => {
  it('filters by website status', async () => {
    ...
    const verified = listBusinessRows({ scope: 'pool', websiteStatus: ['VERIFIED_NO_WEBSITE'] }, db);
  ...
  it('searches across name, city and phone', async () => {
    const { db } = await seedPool();
    expect(listBusinessRows({ scope: 'pool', search: 'melek' }, db)).toHaveLength(1);
    expect(listBusinessRows({ scope: 'pool', search: 'wetzlar' }, db)).toHaveLength(1);
    expect(listBusinessRows({ scope: 'pool', search: '1234502' }, db)).toHaveLength(1);
  });

  it('sorts by the requested column and direction', async () => {
    const { db } = await seedPool();
    const asc = listBusinessRows({ scope: 'pool', sort: 'name', direction: 'asc' }, db).map((r) => r.business.name);
    const desc = listBusinessRows({ scope: 'pool', sort: 'name', direction: 'desc' }, db).map((r) => r.business.name);
  ...
  it('paginates without losing rows', async () => {
    const { db } = await seedPool();
    const total = countBusinessRows({ scope: 'pool' }, db);
    const firstPage = listBusinessRows({ scope: 'pool', limit: 2, offset: 0 }, db);
    const secondPage = listBusinessRows({ scope: 'pool', limit: 2, offset: 2 }, db);
  ...
  it('hides excluded businesses unless asked for', async () => {
    ...
    const target = listBusinessRows({ scope: 'pool' }, db)[0]!;
    excludeBusiness(target.business.id, userId, 'Out of area.', db);

    expect(listBusinessRows({ scope: 'pool' }, db).some((r) => r.business.id === target.business.id)).toBe(false);
    expect(
      listBusinessRows({ scope: 'pool', includeExcluded: true }, db).some((r) => r.business.id === target.business.id),
    ).toBe(true);
  });

  it('counts qualifying and manual-check businesses for the dashboard', async () => {
    const { db } = await seedPool();
    const counts = leadCounts(db);
...
describe('calling', () => {
  async function seedLead() {
    const { db, userId } = await seedPool();
    const qualified = listBusinessRows({ scope: 'pool', qualifiedOnly: true }, db)[0]!;
  ...
  it('records a call without removing the lead', async () => {
    ...
    startCall({ leadId: lead.id, businessId: business.id, userId, phoneE164: business.phoneE164 }, db);

    const rows = listBusinessRows({ scope: 'leads' }, db);
  ...
  it('increments the counter on every attempt', async () => {
    ...
    startCall({ leadId: lead.id, businessId: business.id, userId, phoneE164: business.phoneE164 }, db);
    startCall({ leadId: lead.id, businessId: business.id, userId, phoneE164: business.phoneE164 }, db);
    expect(listBusinessRows({ scope: 'leads' }, db)[0]!.lead?.callCount).toBe(2);
    expect(callHistory(business.id, db)).toHaveLength(2);
  });

  it('stores the outcome, note and callback date', async () => {
    const { db, userId, lead, business } = await seedLead();
    const call = startCall({ leadId: lead.id, businessId: business.id, userId, phoneE164: business.phoneE164 }, db);
    ...
    const history = callHistory(business.id, db);
    expect(history[0]).toMatchObject({ outcome: 'callback', note: 'Ask for the owner.', durationSeconds: 45 });
    expect(listBusinessRows({ scope: 'leads' }, db)[0]!.lead?.nextCallbackAt).toBe(callbackAt);
  });
  ...
  it('keeps a future callback out of the queue until it is due', async () => {
    ...
    expect(callingQueue(userId, 50, db).some((r) => r.lead?.id === lead.id)).toBe(false);
  });

  it('puts a due callback at the front of the queue', async () => {
    const { db, userId } = await seedPool();
    const qualified = listBusinessRows({ scope: 'pool', qualifiedOnly: true }, db);
    expect(qualified.length).toBeGreaterThanOrEqual(2);
    const first = promoteToLead(qualified[0]!.business.id, userId, {}, db);
    const second = promoteToLead(qualified[1]!.business.id, userId, {}, db);
    ...
    const queue = callingQueue(userId, 50, db);
  ...
  it('drops leads with a terminal status from the queue but keeps them in the list', async () => {
    ...
    updateLeadStatus(lead.id, 'not_interested', userId, 'Not a fit.', db);

    expect(callingQueue(userId, 50, db).some((r) => r.lead?.id === lead.id)).toBe(false);
    expect(listBusinessRows({ scope: 'leads' }, db)).toHaveLength(1);
  });

  it('prefers the caller’s own leads over other people’s', async () => {
    const { db, userId } = await seedPool();
    const qualified = listBusinessRows({ scope: 'pool', qualifiedOnly: true }, db);
    const mine = promoteToLead(qualified[0]!.business.id, userId, {}, db);
    const theirs = promoteToLead(qualified[1]!.business.id, userId, {}, db);

    db.prepare(
      `INSERT INTO users (id, email, name, role, password_hash, password_salt, is_active, created_at, updated_at)
       VALUES ('usr_other','other@example.com','Other User','member','x','y',1,?,?)`,
    ).run(Date.now(), Date.now());
    assignLead(mine.id, userId, db);
    assignLead(theirs.id, 'usr_other', db);
  ...
  it('records who made each call', async () => {
    const { db, userId, lead, business } = await seedLead();
    startCall({ leadId: lead.id, businessId: business.id, userId, phoneE164: business.phoneE164 }, db);
    expect(callHistory(business.id, db)[0]?.userName).toBe('Test User');
  });
});
